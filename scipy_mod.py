...
def de_generator(func, bounds, args=(), strategy='best1bin',
    ...
    return ret, solver._scale_parameters(solver.population), solver.population_energies
...
class DifferentialEvolutionSolver:
    ...
    def __init__(self, func, bounds, args=(),
        ...
        if strategy in ['discrete', 'mixed_discrete']:
            self._visited_list = set()
            for i in self.population:
                self._visited_list.add(np.array2string(np.round(self._scale_parameters(i))))
    ...
    def _promote_lowest_energy(self):
        ...
        if feasible_solutions.size:
            ...
            population_rank = np.argsort(self.population_energies[feasible_solutions])
            ...
            population_rank = np.argsort(np.sum(self.constraint_violation, axis=1))
        ...
        '''
        self.population_energies[idx] = self.population_energies[population_rank]
        self.population[idx, :] = self.population[population_rank, :]
        self.feasible[idx] = self.feasible[population_rank]
        self.constraint_violation[idx, :] = (
        self.constraint_violation[population_rank, :])
    ...
    def _discrete(self, samples):
        tries = 0
        rng = self.random_number_generator
        while True:
            lims = np.round(self.limits)
            bprime = self._unscale_parameters(np.random.randint(lims[0], lims[1]+1, lims.shape[1]))
            perturb = rng.uniform(size=self.parameter_count)
            perturb = perturb < self.scale
            
            if tries > 5:
                bprime = np.where(perturb, bprime, np.random.random(self.parameter_count))
            else:
                bprime = np.where(perturb, bprime, self.population[0])
           
            trial_vector_string = np.array2string(np.round(self._scale_parameters(bprime)))

            if trial_vector_string not in self._visited_list:
                self._visited_list.add(trial_vector_string)
                break
            tries += 1
        return bprime

    def _mixed(self, samples):
        if np.random.random() < 0.5:
            return self._discrete(samples)
            r0, r1 = samples[:2]
            bprime = (self.population[0] + self.scale *
                    (self.population[r0] - self.population[r1]))
            return np.round(bprime)
        else:
            return np.round(self._best2(samples))
